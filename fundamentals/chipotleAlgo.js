...
function measureWaterLevels(waterLevels) {
    let diff = 0
    for (let index = 0; index < waterLevels.length; index++) {
        let lowLevel = waterLevels[index], rise;
        while (waterLevels[index] < waterLevels[index + 1]) {
            rise = waterLevels[index + 1]
            index++;
        }
        rise - lowLevel > diff ? diff = rise - lowLevel : diff
    }
